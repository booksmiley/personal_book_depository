...
Async: the sources are awaited so the event loop keeps serving other requests
(borrow/return) while a lookup waits on the network.
"""
...
async def fetch_book_metadata(isbn: str) -> Book | None:
...
async def resolve_open_lib_authors(authors_refs: list) -> str:
    authors = []
...
async def _from_open_library(isbn: str) -> Book | None:
    ...
    try:
        authors = await resolve_open_lib_authors(authors_refs=data.get("authors", []))
...
async def _from_google_books(isbn: str) -> Book | None:
    info = await _google_books_volume(isbn)
    ...
    if info is None:
        ...
        if alt:
            info = await _google_books_volume(alt)
    if info is None:
        return None
    return Book(
...
async def _google_books_volume(isbn: str) -> dict | None:
...
async def _from_isbnnet(isbn: str) -> Book | None:
    data = await fetch_isbnnet_metadata(isbn)
    if data is None:
        return None
    return Book(
        isbn=isbn,
        title=data["title"],
        author=data["author"],
        cover_url=await _cover_url(isbn),  # ISBNnet covers are unreliable; reuse GB/OL
...
async def _from_douban(isbn: str) -> Book | None:
    data = await fetch_douban_metadata(isbn)
    if data is None:
        return None
    return Book(
        isbn=isbn,
...
async def _cover_url(isbn: str) -> str:
...
if __name__ == "__main__":
    ...
    print(asyncio.run(fetch_book_metadata("9780131103627")))
    print(asyncio.run(fetch_book_metadata("9780801020865")))
